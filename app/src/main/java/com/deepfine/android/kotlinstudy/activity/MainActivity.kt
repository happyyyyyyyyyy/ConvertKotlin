...
class MainActivity : AppCompatActivity() {
    ...
    private fun changeScreen(){
        binding.bottomNav.setOnItemSelectedListener {//it: MenuItem == item ->
            when(it.itemId) {
                //홈으로 화면 전환
                R.id.home -> supportFragmentManager.beginTransaction().replace(R.id.mainFrame, HomeFragment()).commit()
                //북마크로 화면 전환
                R.id.bookmark -> {
                    //네트워크 비연결 시 경고창 출력
                    if(Utility().isNetworkConnected(this@MainActivity))
                        showNetworkWarningDialog()
                    supportFragmentManager.beginTransaction().replace(R.id.mainFrame, BookmarkFragment()).commit()
                }
            }
            true
        }
